...
def scraper():
    ...
    if os.path.exists(output_file):
        ...
        new_wb = Workbook()
        new_ws = new_wb.active
        new_ws.append(["标题", "链接", "up主", "up主粉丝数", "up主id", "精确播放数", "历史累计弹幕数", "点赞数", "投硬币枚数", "收藏人数", "转发人数", "发布时间", "抓取时间", "视频时长(秒)", "视频简介", "作者简介", "标签", "视频aid"])
    ...
    for video_id_or_url in id_list:
        ...
        try:
            ...
            # api_response = requests.get(f"https://api.bilibili.com/x/web-interface/archive/stat?bvid={video_id_or_url}",headers=headers)
            # print(api_response)
            # 视频 aid、视频时长和作者 id
            ...
            author_fans_pattern = re.compile(r'"fans":(\d+)')
            ...
            author_fans = author_fans_pattern.search(initial_state_text).group(1)
            ...
            if numbers:
                ...
                new_ws.append([title, url, author, author_fans, author_id, views, danmaku, likes, coins, favorites, shares, publish_date, scrap_time, video_duration, video_desc, author_desc, tags, video_aid])
